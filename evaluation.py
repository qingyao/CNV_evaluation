...
### run CNARA, per series
    arguments = [series, data_dir, update]
...
def weighted_median(pddata,value_colname, weights_colname):
    pddata = pddata.sort_values(value_colname,inplace=True)
    cumsum = (pddata[weights_colname[1]] - pddata[weights_colname[0]]).cumsum()
    cutoff = (pddata[weights_colname[1]] - pddata[weights_colname[0]]).sum() / 2.0
    return (pddata[value_colname])
